...
namespace UI
{
    ...
    public class GraphicMaskable : IGraphicMaskable
    {
        ...
        {
            foreach (var maskable in maskables.Except(cachedMaskable))
            {
        ...
        public void Drop()
        {
            var temp = cachedMaskable.Except(maskables).ToArray();
            foreach (var maskable in temp)
            {
                cachedMaskable.Remove(maskable);
                maskable.Drop();
            }
            cachedMaskable.Clear();
        }
        ...
        public void RemoveMaskable(IGraphicMaskable maskable)
        {
            maskable?.Drop();
            cachedMaskable.Remove(maskable);
        }
