	...
	internal static class SceneAutoLoader
	{
		...
		private static void OnPlayModeChanged(PlayModeStateChange state)
		{
			...
			if (!EditorApplication.isPlaying && !EditorApplication.isPlayingOrWillChangePlaymode)
			{
				...
				string[] scenePaths = AllPrevOpenScenesPaths;
		...
		private static void SetAllPrevOpenScenesPaths()
		{
			string[] result = new string[EditorSceneManager.sceneCount];

			for (var s = 0;  s < result.Length; s++)
			{
				result[s] = EditorSceneManager.GetSceneAt(s).path;
			}
		...
		private const string temp = "temp.dat";
		...
		private static string[] AllPrevOpenScenesPaths
		{
			get
			{
				var formatter = new BinaryFormatter();
